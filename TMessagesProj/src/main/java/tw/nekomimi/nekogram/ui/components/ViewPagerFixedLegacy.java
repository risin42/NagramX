...
public class ViewPagerFixedLegacy extends FrameLayout {
    ...
    public ViewPagerFixedLegacy(@NonNull Context context) {
        this(context, null);
    }

    public ViewPagerFixedLegacy(@NonNull Context context, Theme.ResourcesProvider resourcesProvider) {
        super(context);
    ...
    public TabsView createTabsView(boolean hasStableIds, int selectorType) {
        ...
        tabsView.setDelegate(new TabsView.TabsViewDelegate() {
            ...
            public void onPageSelected(int page, boolean forward) {
                ...
                int translationX = viewPages[0] != null ? viewPages[0].getMeasuredWidth() : 0;
                if (viewPages[1] != null) {
                    if (forward) {
                        setTranslationX(viewPages[1], translationX);
                    } else {
                        setTranslationX(viewPages[1], -translationX);
                    }
                }
            ...
            public void invalidateBlur() {
                ViewPagerFixedLegacy.this.invalidateBlur();
            }
        });
        fillTabs(false);
        return tabsView;
    ...
    protected ValueAnimator translateAnimator(View view, float toTx) {
        ...
        a.addUpdateListener(animation -> setTranslationX(view, (float) animation.getAnimatedValue()));
    ...
    public void fillTabs(boolean animated) {
        if (adapter != null && tabsView != null) {
            ...
            for (int i = 0; i < adapter.getItemCount(); i++) {
                if (adapter.needsTab())
                    tabsView.addTab(adapter.getItemId(i), adapter.getItemTitle());
            }
            if (animated) {
    ...
    private boolean prepareForMoving(MotionEvent ev, boolean forward) {
        if ((!forward && currentPosition == 0) || (forward && currentPosition == adapter.getItemCount() - 1) || manualScrolling != null) {
            return false;
        }
        if (forward && !canScrollForward()) {
            return false;
        }
    ...
    public boolean onTouchEventInternal(MotionEvent ev) {
        ...
        if (ev != null && ev.getAction() == MotionEvent.ACTION_DOWN && !startedTracking && !maybeStartTracking) {
            ...
            if (startedTracking) {
                ...
                if (nextPosition < 0) {
                    ValueAnimator backAnimator = ValueAnimator.ofFloat(backAnimation ? 0f : 1f);
    ...
    public void drawForBlur(Canvas blurCanvas) {
        for (View viewPage : viewPages) {
            if (viewPage != null && viewPage.getVisibility() == View.VISIBLE) {
                RecyclerListView recyclerListView = findRecyclerView(viewPage);
                if (recyclerListView != null) {
                    for (int j = 0; j < recyclerListView.getChildCount(); j++) {
                        ...
                        if (child.getY() < dp(203) + dp(100)) {
                            ...
                            blurCanvas.translate(viewPage.getX(), getY() + viewPage.getY() + recyclerListView.getY() + child.getY());
    ...
    public static class TabsView extends FrameLayout {
        ...
        private final int tabLineColorKey = Theme.key_profile_tabSelectedLine;
        private final int activeTextColorKey = Theme.key_profile_tabSelectedText;
        private final int unactiveTextColorKey = Theme.key_profile_tabText;
        private final int backgroundColorKey = Theme.key_actionBarDefault;
        ...
        public TabsView(Context context, boolean hasStableIds, int tabsSelectorType, Theme.ResourcesProvider resourcesProvider) {
            ...
            int selectorColorKey = Theme.key_profile_tabSelector;
                    ...
                    LinearSmoothScroller linearSmoothScroller = new LinearSmoothScroller(recyclerView.getContext()) {
                        ...
                        protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
                            ...
                            if (dx > 0 || dx == 0 && targetView.getLeft() - dp(21) < 0) {
                                dx += dp(60);
                            } else if (dx < 0 || targetView.getRight() + dp(21) > getMeasuredWidth()) {
                                dx -= dp(60);
                            }
        ...
        public void setAnimationIndicatorProgress(float value) {
        ...
        public void addTab(int id, CharSequence text) {
            ...
            allTabsWidth += tab.getWidth(textPaint) + dp(tabMarginDp * 2);
            tabs.add(tab);
        }
            ...
            if (child == listView) {
                ...
                if (animatingIndicator || manualScrollingToPosition != -1) {
                    ...
                    if (holder != null) {
                        ...
                        indicatorX = (int) (tabView.getX() + (float) (tabView.getMeasuredWidth() - indicatorWidth) / 2);
                    }
                }
                indicatorX += (int) listView.getX();
                if (indicatorWidth != 0) {
                    if (selectorType == 9) {
        ...
        public interface TabsViewDelegate {
            ...
            default boolean needsTab(int page) {
                return true;
            }
        }
        ...
        public class TabView extends View {
            ...
            private void setTab(Tab tab, int position) {
                currentTab = tab;
                currentPosition = position;
            ...
            @Override
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                int w = currentTab.getWidth(textPaint) + dp(tabMarginDp * 2) + additionalTabWidth;
            ...
            protected void onDraw(@NonNull Canvas canvas) {
                if (currentTab.id != Integer.MAX_VALUE) {
                    canvas.save();
                    float p = currentPosition % 2 == 0 ? 1.0f : -1.0f;
                    canvas.translate(dp(0.66f) * p, 0);
                    canvas.rotate(p, (float) getMeasuredWidth() / 2, (float) getMeasuredHeight() / 2);
                }
                int key;
                int otherKey;
                int unreadKey;
                ...
                if (currentTab.id != Integer.MAX_VALUE && (isEditing)) {
                    countWidth = countWidth + (dp(20) - countWidth);
                }

                tabWidth = currentTab.titleWidth + (countWidth != 0 ? countWidth + dp(6 * (counterText != null ? 1.0f : 0)) : 0);
                ...
                if (textLayout != null) {
                    canvas.save();
                    canvas.translate(textX + textOffsetX, (float) (getMeasuredHeight() - textHeight) / 2 + 1);
                    textLayout.draw(canvas);
                    canvas.restore();
                }

                if (counterText != null || currentTab.id != Integer.MAX_VALUE && (isEditing)) {
                    ...
                    if (currentTab.id != Integer.MAX_VALUE && (isEditing) && counterText == null) {
                        counterPaint.setAlpha(0);
                    } else {
                    ...
                    if (counterText != null) {
                        if (currentTab.id != Integer.MAX_VALUE) {
                            textCounterPaint.setAlpha((int) (255 * (1.0f)));
                        }
